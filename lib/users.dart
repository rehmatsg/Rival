...
class RivalRootUser {
  ...
  /// Return [bool]
  bool get manuallyApprovePartnerRequests => data['manuallyApprovePartnerRequests'] ?? true;
  ...
  Map get allStories => data['story'] ?? {};

  /// Map of all valid stories
  /// Valid means that it was created withing last 24hrs
  /// Keys in map are timestamp
  /// Values are story data
  Map get stories {
    Map s = {};
    allStories.forEach((timestamp, data) {
      int t = int.parse(timestamp.toString());
      DateTime created = DateTime.fromMillisecondsSinceEpoch(t);
      DateTime now = DateTime.now();
      if (created.isAfter(now.subtract(Duration(hours: 24)))) {
        s[timestamp] = data;
      }
    });
    return s;
...
class RivalUser extends RivalRootUser {
  ...
  bool get isFollowingMe {
    ...
    me.followers.forEach((u) {
      DocumentReference r = u;
      followersUid.add(r.id);
    });
    if (followersUid.contains(uid)) return true;
    else return false;
  }
  String get followUnfollow {
    if (isFollowing) {
      // Unfollow
      return 'Following';
    } else if (!isFollowing && !isInWaitList && data['allow_new_followers'] && !isFollowingMe) {
      // Follow
  ...
  Future<void> followUnfollowRequest() async {
    if (isBlocked || amIBlocked) {
      RivalProvider.showToast(text: '@${username} has blocked you');
    } else if (isFollowing) {
      // Unfollow
  ...
  List<String> getAllPostsUid() {
    List<String> l = [];
    for (DocumentReference ref in this.posts) {
      l.add(ref.id);
    }
    return l;
  }
