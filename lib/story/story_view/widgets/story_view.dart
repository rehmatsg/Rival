...
class StoryItem {
  ...
  static StoryItem text(
    ...
    return StoryItem(
      ...
      shown: shown,
      duration: duration ?? Duration(seconds: 3), story: story,
    );
  }
              ...
              child: Align(
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      caption != null
                      ? Text(caption, style: TextStyle(fontSize: 15, color: Colors.white), textAlign: TextAlign.center,)
                      : SizedBox(),
                      bottomWidget ?? Container()
                    ],
                  ),
                    ...
                    FutureBuilder<Post>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.done) {
                          return CustomPostView(id: postId, controller: controller);
                        }
      ...
      child: Stack(
        children: <Widget>[
          ...
          Align(
            ...
            child: SafeArea(
              bottom: true,
              // we use SafeArea here for notched and bezeless phones
                ...
                StreamBuilder<PlaybackState>(
                  ...
                  builder: (context, snapshot) {
                    return Column(
                      children: [
                        AnimatedOpacity(
                          duration: snapshot.data == PlaybackState.play ? Duration(milliseconds: 0) : Duration(milliseconds: 500),
                        ...
                        DelayedDisplay(
                          delay: snapshot.data == PlaybackState.play ? Duration(milliseconds: 0) : Duration(milliseconds: 50),
          ...
          Align(
              ...
              child: SizedBox(
                ...
                child: GestureDetector(
                  child: Container(
                    color: Colors.redAccent.withOpacity(0), // Turn up the opacity to see hot area
                  ),
                  onTapDown: (details) {
                    widget.controller.pause();
                  },
          ...
          Align(
            ...
            child: SizedBox(
              child: GestureDetector(
                ...
                child: Container(
                  color: Colors.blueAccent.withOpacity(0), // Turn up the opacity to see hot area
                ),
  ...
  _init() async {
    ...
    if (post != null) {
      if (post.available) {
        setState(() {
          isLoading = false;
          if ((post.isMyPost || (post.user.private && post.user.isFollowing) || post.available) && !post.takenDown) {
            isPostHidden = false;
          }
        });
      } else {
              ...
              if (isPostHidden) ... [
                Padding(
                  ...
                  child: Align(
                    ...
                    child: Text('@${post.user.username} has a private account. Follow @${post.user.username} to view this post')
                  ),
                )
              ] else if (!available) ...  [
                Column(
                  children: [
                    Icon(Icons.warning, color: Colors.yellow, size: Theme.of(context).textTheme.headline3.fontSize,),
                    Text('Post Not Found', style: Theme.of(context).textTheme.headline5.copyWith(color: Colors.white, fontFamily: RivalFonts.feature),),
                  ],
                )
              ] else ... [
                ClipRRect(
            ...
            if (!isPostHidden) {
              showModalBottomSheet(
                ...
                builder: (context) => BottomSheet(
                  onClosing: () { },
                  backgroundColor: MediaQuery.of(context).platformBrightness == Brightness.light ? Colors.white : Colors.grey[900],
                  builder: (context) => Padding(
                    padding: const EdgeInsets.symmetric(vertical: 13, horizontal: 15),
                    child: ListTile(
                      title: Text('View Post'),
                      subtitle: Text('Tap to open post'),
                      onTap: () => Navigator.of(context).pushReplacement(RivalNavigator(page: SinglePostView(post: post),)),
                    ),
                  ),
                  elevation: 0,
                  key: UniqueKey(),
