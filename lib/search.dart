...
import 'package:e/post/post.dart';
  ...
  Widget buildResults(BuildContext context) {
    ...
    if (queryLocal == null || queryLocal == "") {
      ...
      return FutureBuilder(
        future: firestore.collection('posts').where('keywords', arrayContains: queryLocal.substring(1, queryLocal.length)).get(),
      ...
      // Search for people
      String query = queryLocal.startsWith('@') ? queryLocal.substring(1, queryLocal.length) : queryLocal;
      return FutureBuilder(
        future: firestore.collection('users').where('username', isEqualTo: query).get(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.done) {
            QuerySnapshot querySnapshot = snapshot.data;
            if (querySnapshot.docs.length == 0) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) => UserListTile(doc: querySnapshot.docs[index],),
            );
          }
          return Center(
            child: Column(
