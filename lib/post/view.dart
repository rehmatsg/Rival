...
import '../screens/subscribe_to_topics.dart';
...
class _PostUserState extends State<PostUser> {
  ...
  Widget _getLeadingWidget() {
    if (widget.isCurrentUser) {
      return ProfilePhoto(width: 35, height: 35, hero: false);
    } else {
      return ClipOval(
        child: OctoImage(
          height: 35,
...
//                 snapshot.data.navigateToProfile(context);
//                 if (snapshot.data.uid != me.user.uid && !post.profileVisits.containsKey(me.user.uid)) {
//                   await post.reference .update({
...
class _ViewPostState extends State<ViewPost> {
  ...
  Widget build(BuildContext context) {
    ...
    } else if (post.beta && !kDebugMode) { // This post was created in debug mode, so it should not be available to public
      return InfoBanner(
        leadingIcon: Icons.error,
                  ...
                  child: SizedBox(
                    ...
                    child: Stack(
                      children: [
                          ...
                          itemBuilder: (context, index) {
                            switch (post.items[index].type) {
                              case PostType.image:
                                ...
                                break;
                              case PostType.poll:
                                PollData poll = post.items[index].poll;
                                int totalVotes = List<int>.generate(poll.options.length, (index) => poll.options[index].votes.length).fold(0, (a, b) => a + b);
                                return Stack(
                                  children: [
                                    Align(
                                          ...
                                          ... List.generate(
                                            ...
                                            (index) => GestureDetector(
                                              ...
                                              child: Container(
                                                ...
                                                child: Stack(
                                                  children: [
                                                    if (poll.options[index].votes.containsKey(me.uid)) SizedBox(
                                                      height: 50,
                                                      child: ClipRRect(
                                                        borderRadius: BorderRadius.all(Radius.circular(10)),
                                                        child: LinearProgressIndicator(
                                                          value: 0.6,
                                                          valueColor: AlwaysStoppedAnimation(Colors.indigoAccent.withOpacity(0.3)),
                                    ...
                                    Align(
                                      ...
                                      child: Padding(
                                        ...
                                        child: Text(
                                          '${totalVotes > 0 ? totalVotes : 'No'} votes',
                        ...
                        if (post.items.length > 1) Align(
                          alignment: Alignment.bottomRight,
                          child: Padding(
                            padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 10),
                            child: Container(
                              child: PageNotifier(controller: _pageController, pages: post.items.length,),
                            ),
                          ),
                        )
                      ],
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          if (post.allowComments || post.comments.isNotEmpty) IconButton(
                            padding: EdgeInsets.zero,
                            visualDensity: VisualDensity.compact,
