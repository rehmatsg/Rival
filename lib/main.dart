      ...
      return FutureBuilder(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.done && !snapshot.hasError) {
            //getMyPosts();
            //getTopPosts();
            if (me.devices.isEmpty || (!me.devices.containsKey(me.loginTimestamp))) {
              RivalProvider.showToast(text: 'You have been logged out. Please login again.');
            ...
            RivalProvider.showToast(text: 'Failed to login. Unable to connect to server');
          }
          return Scaffold(
            body: Center(
              child: Column(
...
class _HomeState extends State<Home> {
  ...
  void initState() {
    ...
    // 
    helper.addListener(onHandleUpdate);
    super.initState();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => Navigator.of(context).push(RivalNavigator(page: CreatePost())),
  ...
  Widget _buildPosts() {
          ...
          child: ListTile(
            leading: ClipRRect(
              ...
              child: OctoImage(
                ...
                width: 40,
                image: FileImage(
                  helper.queue[index].items.firstWhere((item) => item['type'] == 'image')['file']
                ),
              ),
            ),
            title: Text('Creating Post'),
            subtitle: Text(helper.queue[index].state ?? '...'),
            trailing: SizedBox(
              height: 25,
              width: 25,
              child: CustomProgressIndicator(
    ...
    if (RivalRemoteConfig.showGoogleAds) {
      if (widgets.length >= 2) widgets.insert(1, GoogleAd());
      if (widgets.length >= 6) widgets.insert(6, GoogleAd());
    }
    return SliverList(
  ...
  void onHandleUpdate() {
    try {
      ...
      print('Failed to setState on helper update');
    }
  }
...
class GoogleAd extends StatefulWidget {
  ...
  GoogleAd({Key key}) : super(key: key);
...
class _GoogleAdState extends State<GoogleAd> {
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      height: isLoaded ? ad.size.height.toDouble() : 0,
      duration: Duration(milliseconds: 300),
      alignment: Alignment.center,
      child: isLoaded ? IgnorePointer(
        ignoring: hasClicked,
        child: AdWidget(
          ad: ad,
        ),
