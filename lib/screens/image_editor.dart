...
class _RivalImageEditorState extends State<RivalImageEditor> {
          ...
          actions: [
            IconButton(
              ...
              onPressed: () async {
                ...
                String path = (await getTemporaryDirectory()).path + "/${new DateTime.now().millisecondsSinceEpoch}.png";
                ...
                child: OctoImage(
                  ...
                  placeholderBuilder: (context) {
                    return Container(
                      ...
                      height: (MediaQuery.of(context).size.width - 20) / (width / height),
              ...
              child: Column(
                children: [
                  Slider(
                    min: 0,
                    max: 2,
                    label: 'Brightness  ${(brightness/2*100).toInt()}%',
                    onChanged: (value) {
                      setState(() {
                        brightness = value;
                      });
                  ...
                  Slider(
                    min: 0,
                    max: 2,
                    label: 'Saturation ${(saturation/2*100).toInt()}%',
                    onChanged: (value) {
                      setState(() {
                        saturation = value;
                      });
  ...
  _flipImage({bool horizontal = false, bool vertical = false}) async {
    ...
    editorOption.addOptions(<Option>[
      FlipOption(horizontal: horizontal, vertical: vertical)
    ]);
    Uint8List u8L = await ImageEditor.editImage(image: u8l, imageEditorOption: editorOption);
    setState(() {
      u8l = u8L;
    });
  }
  ...
  Future<void> _applyEffects() async {
    ...
    Uint8List u8L = await ImageEditor.editFileImage(file: file, imageEditorOption: editorOption);
    setState(() {
      u8l = u8L;
    });
  }
  ...
  Future<void> _addText() async {
            ...
            body: SafeArea(
              ...
              child: Container(
                child: TextEditor(
                  ...
                  text: 'Hello World',
                  textStyle: TextStyle(fontFamily: RivalFonts.feature, color: Colors.white, fontSize: 50),
                  textAlingment: TextAlign.center,
                  onEditCompleted: (style, align, text, textPlacement) {
                    Navigator.of(context).pop([style, align, text, textPlacement]);
                  },
                ),
              ),
            ),
    ...
    if (list != null) {
      ...
      double textRatio = textStyle.fontSize / MediaQuery.of(context).size.width; // Ratio of text on device's screen
      //double imageRatio = width / height; // Ratio of Image

      double fontSize = textRatio * width; // If size of text per pixel is `x`, then size for image width is `x * imageRatio * device pixel ratio`
      ...
      if (textPlacement == TextPlacement.top) {
        ...
        dY = (height - calcHeightOfText(text, fontSize) / 2) - (width / 30); // Subtract fontSize from height and remove some area for padding below
      }

      if (alignment == TextAlign.center) {
        dX = (width - calcWidthOfText(text, fontSize)) / 2;
      } else if (alignment == TextAlign.right) {
        dX = (width - calcWidthOfText(text, fontSize)) - (width / 30); // Keep a padding of (width / 30) on right side
      ...
      textOption.addText(
        EditorText(
          ...
          fontName: fontsByManager[textStyle.fontFamily], // You must register font before use.
  ...
  Future<void> loadAllFonts() async {
    for (String font in fonts.values.toList()) {
      ...
      String fontReadableName = fonts.keys.toList()[fonts.values.toList().indexOf(font)];
      fontsByManager[fontReadableName] = await FontManager.registerFont(fontFile);
    }
  }
      ...
      body: ListView(
        ...
        children: [
                ...
                onPressed: () async {
                  try {
                    ...
                    if (await storage.isRestricted) {
                      ...
                      await RivalProvider.showToast(text: 'Permission Denied');
                      return;
                    }
                    ...
                    if (await storage.isGranted) {
                      ...
                      String result = await ImageGallerySaver.saveImage(u8l, quality: 100);
                      try {
                        File finalFile = File(result.allAfter('file://')); // Result will return path to file
          ...
          ButtonBar(
            ...
            children: [
              FlatButton.icon(
                label: Text('Flip Vertical', style: TextStyle(color: Colors.white),),
                icon: Icon(Icons.flip_to_front_outlined, color: Colors.white,),
                onPressed: () {
                  setState(() {
              ...
              FlatButton.icon(
                label: Text('Flip Horizontal', style: TextStyle(color: Colors.white),),
                icon: Icon(Icons.flip_to_back_outlined, color: Colors.white,),
                onPressed: () {
                  setState(() {
          ...
          ExpandablePanel(
            ...
            collapsed: ListTile(
              title: Text('Adjust', style: Theme.of(context).textTheme.bodyText1.copyWith(fontFamily: RivalFonts.feature, color: Colors.white)),
              trailing: IconButton(
            ...
            expanded: Column(
              children: [
                ...
                Padding(
                  ...
                  child: Text('Brightness', style: Theme.of(context).textTheme.bodyText2.copyWith(color: Colors.white)),
                ),
                Slider.adaptive(
                  value: brightness,
                  label: 'Brightness',
                ...
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 15),
                  child: Text('Contrast', style: Theme.of(context).textTheme.bodyText2.copyWith(color: Colors.white)),
                ),
                Slider.adaptive(
                  value: contrast,
                  label: 'Contrast',
                ...
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 15),
                  child: Text('Saturation', style: Theme.of(context).textTheme.bodyText2.copyWith(color: Colors.white)),
                ),
                Slider.adaptive(
                  value: saturation,
                  label: 'Saturation',
...
class NumberSelector extends StatefulWidget {
  ...
  const NumberSelector({Key key, @required this.min, @required this.max, @required this.onSelect, this.scrollAxis = Axis.vertical, @required this.defaultValue}) : assert(min != null && max != null, "Min, Max or Default Value should not be [NULL]"), assert(onSelect != null, "Function Onselect should not be null"), super(key: key);
        ...
        child: CarouselSlider(
          ...
          items: List.generate(
            ...
            (index) => Container(
              ...
              height: 60,
              child: Center(
                child: Text('${values[index]}', style: TextStyle(fontFamily: RivalFonts.feature, color: Colors.white, fontSize: 25))
              ),
            )
          ),
          options: CarouselOptions(
            ...
            height: 50,
            aspectRatio: 1,
            initialPage: values.contains(widget.defaultValue) ? values.indexOf(widget.defaultValue) : 0,
...
class Filter {
  ...
  Filter({this.brightness, this.contrast, this.saturation, this.name, this.whites, this.blacks});
